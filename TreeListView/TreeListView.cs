...
namespace System.Windows.Forms
{
	...
	public class TreeListView : System.Windows.Forms.ListView
	{
		...
		private void autoResizeListViewColumn(ColumnHeader CH)
		{
			...
			foreach (TreeListViewItem LVI in Items)
			{
				int temp = 0;
				if (CH.Index == 0)
					temp = LVI.Text.Length;
				else
					temp = LVI.SubItems[CH.Index].Text.Length;
			...
			if (changeHeader)
				AutoResizeColumn(CH.Index, ColumnHeaderAutoResizeStyle.HeaderSize);
			else
				AutoResizeColumn(CH.Index, ColumnHeaderAutoResizeStyle.ColumnContent);
		}
		...
		#region WndProc
		DateTime starttime;
		APIsEnums.WindowMessages lastmsg = 0;
		...
		{
			//stop messages which dont go away after X seconds
			if (lastmsg != (APIsEnums.WindowMessages)m.Msg)
			{
				lastmsg = (APIsEnums.WindowMessages)m.Msg;
